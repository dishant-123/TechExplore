...
import GadgetDiscription from './GadgetDiscription';
...
class Gadget extends Component {
    ...
    handleLike = async (id) =>{
        ...
        if(logginDetails!=null){
            ...
            const response = await axios.patch(`http://localhost:3001/gadgetDetails/gadgetDescription/likeHandle/${id}`,user);//handle like of each description for each users.
     ...
     handleClick = (event) =>{
        ...
        if(isLogin == true)
        {
            ...
            // if(userLikesResponse !== null || userLikesResponse !== '')
            // {
                var userLikesResponse = await axios.get(`http://localhost:3001/gadgetDetails/activeUserLikes/${logginDetails.email}`);
                let response = await axios.get(`http://localhost:3001/gadgetDetails`);//all  description details(for login and also for logout)
                if(response.data){
                   this.setState({
                       content: response.data,//sabhi description ka content aa jaye
        ...
        return (
         ...
         this.state.content.length!=0? (this.state.content.map((value,index) => 
             <div key ={index} className = "mainrdiv">
                ...
                {this.state.goToDisciption===true  && value._id ===this.state.Discription._id  ?<GadgetDiscription id = {this.state.Discription._id} /> : null}
            </div>//map end.
