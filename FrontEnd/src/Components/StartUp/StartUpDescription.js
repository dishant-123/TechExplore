...
class StartUpDescription extends Component {
    ...
    handleSubCommentReply = async (event) =>{
        ...
        if(logginDetails!=null){
            ...
            const response  = await axios.post(`http://localhost:3001/startUpDetails/postSubComments`,ids);
    ...
    handleSubmit =async (event) =>{
        ...
        if(logginDetails!=null){
            ...
            const response  = await axios.patch(`http://localhost:3001/startUpDetails/startUpDescriptionComments/${this.state.id}`,user);
            // console.log(response.data.descDetails);
            if(response.data.descDetails!=null){
                    this.setState({
                        content : response.data.descDetails,
    ...
    async componentWillMount(){
        // console.log('mount');
        const response = await axios.get(`http://localhost:3001/startUpDetails/getDescriptionDetails/${this.state.id}`);
        // console.log(response.data.descDetails);
             ...
             (
                ...
                <h3>Comments</h3>           
                {
                                        ...
                                        <font onClick ={() =>this.handleReply(details._id)} style={{color:'blue'}}>reply</font><br/>
                                        {
                                            ...
                                            (
                                                ...
                                                <ul>
                                                    ...
                                                    <textarea type="text" row="10" cols="10" onChange={this.handleChange} value={this.state.subComment} type="text" name="subComment" placeholder="Type your reply here" required/>
                    ...
                    <form className="" onSubmit = {this.handleSubmit}>
                            <div className="">
                            <textarea id="text-area"row="10" cols="80" onChange={this.handleChange} value={this.state.comment} type="text" name="comment" placeholder="Type your comment here" required/>
                            </div>
                            <div className = "wrap-login100-form-btn">
